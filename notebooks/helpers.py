...
def get_train_and_test_list():
    test_path = remove_suffix(get_config()["DATA_FOLDER"], 'images/') + 'test_list.txt'
    with open(test_path) as f:
        test_names = f.read().splitlines()
    with open(remove_suffix(get_config()["DATA_FOLDER"], 'images/') + 'train_val_list.txt') as f:
        train_names = f.read().splitlines()
    return test_names, train_names


def get_data_subset(n_test, n_train, random_state=None):
    if random_state is None:
        random_state = get_config()["RANDOM_STATE"]

    random.seed(random_state)

    train_list, test_list = get_train_and_test_list()

    if n_train < 1 and n_test < 1 and n_train + n_test == 1:
        n_train = len(train_list) // n_train
        n_test = len(test_list) // n_test

    return random.sample(train_list, n_train), random.sample(test_list, n_test)


def get_image_by_filename(filename, directory):
...
def flatten_data() -> None:
    """
    Moves all files in CONFIG["TARGET_FOLDER"] to CONFIG["FLATTENED_DATA_DIR"]
    :return: None
    """
    try:
        config = get_config()
    ...
    raw_data_dir = config["TARGET_FOLDER"]
    flattened_data_dir = config["FLATTENED_FOLDER"]

    files = glob.glob(raw_data_dir + '**/**/*.png', recursive=True)
